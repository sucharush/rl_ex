# Continuous-State Reinforcement Learning for Rectangle Alignment
...
The agent acts in a **continuous 3D action space**
\( a = $[\Delta x, \Delta y, \Delta \theta]$ \),
where each component represents local translations and rotations of the rectangle.
Actions are bounded and scaled by maximum step sizes
\($[s_x, s_y, s_\theta]$ = [1.2, 1.2, $\pi/4$]\), ensuring consistent motion magnitudes within the bounded region \($[-10, 10]^2$\).
...
- **Visualization**: Matplotlib-based rendering and rollout GIFs for qualitative evaluation
- **Action scaling**: \([$s_x$, $s_y$, $s_\theta$] = [1.2, 1.2, $\pi/4$]\)
---

##  Results Summary
| Stage | Description | Avg Reward | Avg Episode Length |
|--------|--------------|-------------|--------------------|
| 1 | Baseline training | −0.05 ± 6.41 | 9.66 ± 10.44 |
