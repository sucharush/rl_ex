...
def compute_safe_limits(rect_params, offset_range):
    ...
    x_lim = (int(np.floor(-(max_extent + max_dx))),
            int(np.ceil(max_extent + max_dx)))
    y_lim = (int(np.floor(-(max_extent + max_dy))),
            int(np.ceil(max_extent + max_dy)))

    return x_lim, y_lim
def build_env(rect_params, optimizer_params, points_params, x_lim, y_lim):
    R = Rectangle(**rect_params)
    points = R.sample(**points_params)
...
def make_env_generator(
    ...
    def _generator():
        ...
        return build_env(rect_params, optimizer_params, pp, x_lim, y_lim)
